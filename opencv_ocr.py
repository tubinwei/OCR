...
class opencv_ocr(object):
    ...
    def _svm_classify_string(self, model, img_list):
        str0 = ''
        for img in img_list:
            str0 = str0 + self._svm_classify_one_char(model, img)
        return str0
    ...
    def __scall_and_border_image(self, src_image, target_size):
        ...
        _ratio_image = float(w) / float(h)
        _max = max(_ratio_image, _ratio_target)
        if _max == _ratio_image:
            w1 = target_size[0]
            h1 = max(int(target_size[0] / _ratio_image),1)
        else:
            w1 = max(int(target_size[1] * _ratio_image),1)
            h1 = target_size[1]
        _src_image = cv2.resize(src_image, (w1, h1), interpolation=cv2.INTER_LINEAR)

        _left = (target_size[0] - w1) // 2
        _right = target_size[0] - w1 - _left
        _top = (target_size[1] - h1) // 2
        _bottom = target_size[1] - h1 - _top
        _src_image = cv2.copyMakeBorder(_src_image, _top, _bottom, _left, _right, cv2.BORDER_CONSTANT, value = 0)

        return _src_image, (_left, _top, float(w1)/float(w), float(h1)/float(h))