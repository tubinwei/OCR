...
if __name__ == '__main__' and __package__ is None:
    ...
    for index, _path in enumerate(images_path):
        ...
        # 字符串精确定位，包括歪斜修正
        img_binary = _opencv_ocr._character_location(image,Precise=False)
        # 分割每一个字符
...
#        _opencv_ocr._correct_char_image(_image_list, (_median_h-5, _median_h+5))
        ...
        string = _opencv_ocr._svm_classify_string(_svm_model, _image_list)
        print ('classify %s is :'%_path, string)



        # 输出单个字符图片用于训练
        for __idx, (__img, _ch) in enumerate(zip(_image_list, string)):
